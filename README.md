...
| [`apppromovideo`](Casks/apppromovideo.rb) | [Outcasts AppPromoVideo](https://github.com/betyourluck/AppPromoVideo) — turns an app repository and UI snapshots into prompts and reference images for video-generation AIs |
...
**macOS is Apple Silicon only.** There is no Intel build. All three apps are
signed with a Developer ID certificate and notarized by Apple, so they open
without a Gatekeeper prompt — Fuseforks from v0.1.9, Lorekeel from v0.5.16 (as
`kataribe`), AppPromoVideo from v0.1.1. Earlier releases of any of them carry
only a linker ad-hoc signature and are refused once Homebrew applies its
quarantine.
...
downloaded packages and API keys for Lorekeel, your runs (prompts, reference
images) and image-generation API keys for AppPromoVideo. `brew uninstall --zap
--cask <name>` deletes that too — see the `zap` stanza in each cask for the exact
paths.
...
| [`apppromovideo`](Casks/apppromovideo.rb) | [Outcasts AppPromoVideo](https://github.com/betyourluck/AppPromoVideo) — アプリのリポジトリと UI スナップショットから、動画生成 AI に貼るプロンプトと参照画像を出す |
...
**macOS は Apple Silicon 専用**で、Intel 版はありません。3 つのアプリはいずれも
Developer ID で署名され Apple の公証を通しているので、Gatekeeper に止められずに
開きます（Fuseforks は v0.1.9 から、Lorekeel は v0.5.16 から＝当時の名前は
`kataribe`、AppPromoVideo は v0.1.1 から）。それ以前の版はリンカのアドホック署名
しか持たないため、Homebrew が付ける検疫属性の下では拒否されます。

`brew uninstall --cask <名前>` はアプリだけを消し、**データは残します** —
Fuseforks は村（サーヴァント・会話・作業フォルダ）、Lorekeel はセーブデータ・
取得したパッケージ・API キー、AppPromoVideo は run（プロンプトと参照画像）と
画像生成の API キーです。`--zap` を付けるとそれらも消えます。
